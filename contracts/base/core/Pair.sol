...
contract Pair is IERC20, IPair, Reentrancy {
    ...
    function _update0(uint amount) internal {
        uint toTreasury = (amount * IFactory(factory).TREASURY_FEE()) /
            IFactory(factory).DOMINATOR();
    ...
    function swap(
        ...
        );
        {
            ...
            if (amount0In > 0) {
                _update0(
                    (amount0In * IFactory(factory).SWAP_FEE()) /
                        IFactory(factory).DOMINATOR()
                );
            }
            ...
            if (amount1In > 0) {
                _update1(
                    (amount1In * IFactory(factory).SWAP_FEE()) /
                        IFactory(factory).DOMINATOR()
                );
            }
    ...
    function getAmountOut(
        ...
        amountIn -=
            (amountIn * IFactory(factory).SWAP_FEE()) /
            IFactory(factory).DOMINATOR();
